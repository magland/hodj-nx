...
const panResolver: DebounceThrottleResolver<PanUpdateRefs, PanResolverProps> = (refs, props) => {
    const {panStateRef} = refs
    const {secondsPerPixel, panTimeSelectionDeltaT} = props
    ...
    panTimeSelectionDeltaT && panTimeSelectionDeltaT(secondsPerPixel * deltaPx)
    // Don't reset panning state by default here--user may still be holding the mouse button
}


export const useThrottledPan = (refs: PanUpdateRefs, secondsPerPixel: number, panTimeSelectionDeltaT: PanFn) => {
    const resolverProps = useMemo(() => {return {secondsPerPixel, panTimeSelectionDeltaT}}, [secondsPerPixel, panTimeSelectionDeltaT])
